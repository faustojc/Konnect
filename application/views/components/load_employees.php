...
if (!empty($employees)) {
    foreach ($employees as $index => $employee) {
        ?>
        <a href="<?php echo base_url() ?>employee_profile/index/<?= $employee->ID ?>" class="card card-hover my-3 sec-color" style="border-radius:15px;  box-shadow: unset; height:80px; text-decoration: none;">
            <div class="card-body card-hover border-0 py-2" style="border-radius:15px; ">
                <div class="widget-user-header  <?php if ($index + 1 > 1) echo 'border-0'; ?>" style="padding:0.2rem;">
                    ...
                    <div class="d-flex justify-content-between">
                        <span class="card-title text-truncate text-info" style="width: 250px; margin-left: 10px;"><?= ucwords($employee->Fname) . ' ' . ucwords($employee->Lname) ?></span>
